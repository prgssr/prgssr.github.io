...
Очевидно, что сначала надо установить нужные gemы - `gem install kramdown maruku rdiscount redcarpet`. Это может занять время, так как RDiscount и Redcarpet используют расширения C для ускорения парсинга.
