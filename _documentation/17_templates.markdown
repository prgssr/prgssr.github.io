...
Все остальны возможности тега `include`, такие как использование переменных, полностью  доступны в `include_relative`.

### Подсветка фрагментов кода

В Jekyll благодаря [Pygments](http://pygments.org/)реализована подсветка синтаксиса для [более чем 100 языков](http://pygments.org/languages/). Для использования Pygments вам надо установить в вашйе системе Python  и задать в конфигурации сайта для настройки `highlighter` значение `pygments`.
...
Вы также можете использовать этот тего для создания ссылок в разметке Markdown:
