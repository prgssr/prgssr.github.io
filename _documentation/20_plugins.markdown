...
Например, генератор может вставить значения расчитанные в моментсборки для переменных шаблона. В следующем примере в `reading.html` есть две переменные, `ongoing` и `done`, которые мы заполняем в генераторе:
...
Если у вас есть новый язык разметк, который вы хотите использовать на сайте, вы можете подключить его, создав свой плагин-конверетер. Markdown и Textile подключены именно таким образом.
