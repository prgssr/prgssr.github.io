...
* `no_fenced_code_blocks` - по умолчанию  Jekyll подключает расширение `fenced_code_blocks` (для выделения блоков кода тройными или тройными обратными кавычками), вероятно, потому что использование их на GitHub сделало эти блоки стандартом де-факто. Расширение Redcarpet `fenced_code_blocks` инертно при работе с Jekyll; вы можете отключить выделение блоков кода этим способом и вернуть обычное выделение четырьмя пробелами.
